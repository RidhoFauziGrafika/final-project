            ...
            <form action="{{ route('field.update', $field->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" cols="30"
                        rows="5" required>{{ old('deskripsi', $field->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="oldGambar" value="{{ $field->gambar }}">
                    @if ($field->gambar)
                        <img src="{{ asset('storage/' . $field->gambar) }}"
                    ...
                    <input type="file" name="gambar" id="gambar"
                        class="form-control @error('gambar') is-invalid @enderror" required onchange="previewImage()">
                    @error('gambar')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
